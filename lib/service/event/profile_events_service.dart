...
class ProfileEventsStorageService {
  ...
  Future<bool> hasCurrentConfirmed(String eventHash) async {
    var currentProfileHash = UserProvider().getCurrentProfileHash();
    return await ProfileEventStorage().hasProfileConfirmed(eventHash, currentProfileHash);
  }


  Future<Set<String>> profilesThatConfirmed(String eventHash, Set<String> myProfiles) async {
    return await ProfileEventStorage().profilesThatConfirmed(eventHash, myProfiles);
  }

  Future<int> countMatchingProfiles(String eventHash, Set<String> userProfileHashes) async {
    return await ProfileEventStorage().countMatchingProfiles(eventHash, userProfileHashes);
  }
