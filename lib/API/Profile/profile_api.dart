...
class ProfileAPI {
  String? functionUrl = '${ConfigService().backUrl}/wyd/api/Profile/';
  ...
  Future<RetrieveDetailedProfileResponseDto> updateProfile(UpdateProfileRequestDto updateDto) async {
    String url = '${functionUrl}Update';

    var response = await client.post(Uri.parse(url), body: jsonEncode(updateDto));

    if (response.statusCode == 200) {
      return RetrieveDetailedProfileResponseDto.fromJson(json.decode(response.body));
    }

    throw "Error while updating the profile";
  }

  Future<RetrieveDetailedProfileResponseDto> retrieveDetailed(String profileId) async {
